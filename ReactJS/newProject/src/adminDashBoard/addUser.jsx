...
const AddUser = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
        const registerResponse = await axios.post('http://localhost:8080/register', newUser, {
            headers: {
                'Content-Type': 'application/json'
            }
        });

        // Assuming the response contains a token or user ID
        const token = registerResponse.data.token;
        const payload = JSON.parse(atob(token.split('.')[1]));

        const tokenn = sessionStorage.getItem('jwtToken');
        const userResponse = await axios.get(`http://localhost:8080/user/${payload.sub}`, {
            headers: {
